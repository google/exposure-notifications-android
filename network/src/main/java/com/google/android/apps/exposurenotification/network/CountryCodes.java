 ...
 */
class CountryCodes {
  // TODO: This default is only to ease testing while development progresses. Remove it.
   ...
   *
   * <p>This simple implementation returns the current country code from the TelephonyManager, based
   * on MCC. A more complete implementation would retain and return a list of the user's relevant
   * country codes for the past N days (currently 14).
   */
  ListenableFuture<List<String>> getExposureRelevantCountryCodes() {
    ...
    return Futures.immediateFuture(ImmutableList.of(countryCode));
  }
}
