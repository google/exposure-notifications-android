...
/**
 * Tests for operations in {@link PositiveDiagnosisDao}.
 */
@RunWith(RobolectricTestRunner.class)
public class PositiveDiagnosisDaoTest {

  // Sample data with which we'll populate the test database.
  private static final long TEST_TS_0 = 0L;
  private static final boolean SHARED_0 = true;
  ...
  public void positiveDiagnosisDao_upsert() {
    ...
    PositiveDiagnosisEntity entity = createPositiveDiagnosisEntity(createdTs, shared, testedTs);
  ...
  private void populateDbWithTestData() {
    positiveDiagnosisDao
        .upsert(createPositiveDiagnosisEntity(CREATED_TS_0, SHARED_0, TEST_TS_0));
    positiveDiagnosisDao
        .upsert(createPositiveDiagnosisEntity(CREATED_TS_1, SHARED_1, TEST_TS_1));
    positiveDiagnosisDao
        .upsert(createPositiveDiagnosisEntity(CREATED_TS_2, SHARED_2, TEST_TS_2));
  }

  private PositiveDiagnosisEntity createPositiveDiagnosisEntity(
      long createdTs, boolean shared, long testTs) {
    PositiveDiagnosisEntity entity = new PositiveDiagnosisEntity(
        Instant.ofEpochMilli(testTs).atZone(ZoneId.of("UTC")), shared);
